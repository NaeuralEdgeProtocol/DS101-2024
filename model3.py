...
class ImprovedSoccerPredictionModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.result_head = nn.Sequential(
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, 3),  # Predict 3 classes: Home Win, Draw, Away Win
    ...
    def forward(self, x):
        ...
        result = self.result_head(x)  # Softmax output for result classification
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, device):
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Progress"):
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_model(model, 'best_model3.pth')
