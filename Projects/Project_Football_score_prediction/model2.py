...
class ImprovedSoccerPredictionModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.draw_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(32, 1),
            nn.Sigmoid()  # Probability of draw
        )
        ...
        self.loss_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(32, 1),
            nn.Sigmoid()  # Probability of loss
        )
    ...
    def forward(self, x):
        ...
        draw_prob = self.draw_head(x)
        loss_prob = self.loss_head(x)
        
        # Combine probabilities and normalize
        probs = torch.cat([win_prob, draw_prob, loss_prob], dim=1)
        probs = probs / (probs.sum(dim=1, keepdim=True) + 1e-6)
        
        # Get most likely result (-1, 0, 1)
        result = torch.argmax(probs, dim=1).float() - 1
        
        # Return goals and result
        return torch.cat([goals, result.unsqueeze(1)], dim=1)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, device):
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Progress"):
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_model(model, 'best_model2.pth')
...
def main():
    ...
    load_model(model, 'best_model2.pth')
